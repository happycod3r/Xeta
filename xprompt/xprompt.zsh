...
function xeta_git_prompt_info() {

    function xeta_git_status() {
    ...
    function xeta_git_status_prompt() {
    ...
    xeta_git_status
    xeta_git_status_prompt
}

function xeta_set_prompt() {
    local root_icon="$(echo $xicons_nfall[ROOT_ICON])"
    local reset='%f'

    local X_HOST='%n'
    local X_USER='@%m'
    local X_USER_CONTEXT="%124F${X_HOST}%88F${X_USER}"
    local X_CWD='%125F%$((-GITSTATUS_PROMPT_LEN-1))<…<%~%<<%f'
    local X_GIT_STATUS='${GITSTATUS_PROMPT:+ $GITSTATUS_PROMPT}'
    local X_NEWLINE=$'\n'
    local X_PROMPT_CHAR=">"
    local X_OKAY_ERROR='%F{%(?.76.196)}'

    RPROMPT="$X_GIT_STATUS"
