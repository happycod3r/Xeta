...
1. Clone this repository into `$XCUSTOM/plugins` (by default `~/.xeta/custom/plugins`)

    ```sh
    git clone https://github.com/zsh-users/zsh-autosuggestions ${XCUSTOM:-~/.xeta/custom}/plugins/zsh-autosuggestions
    ```

2. Add the plugin to the list of plugins for Xeta to load (inside `~/.zshrc`):
