# diagnostics.zsh
#
# Diagnostic and debugging support for xeta

# xeta_diagnostic_dump()
#
# Author: Andrew Janke <andrew@apjanke.net>
#
# Usage:
#
# xeta_diagnostic_dump [-v] [-V] [file]
#
...
# xeta, zsh, and the user's system. This is intended to provide a
...
#
# This is intended to be widely portable, and run anywhere that xeta does.
...
#       Verbosity levels:
#        0 - Basic info, shell state, xeta configuration, git state
...
function xeta_diagnostic_dump() {
  emulate -L zsh

  builtin echo "Generating diagnostic dump; please be patient..."
  
  local thisfcn=xeta_diagnostic_dump
  local -A opts
  local opt_verbose opt_noverbose opt_outfile
  local timestamp=$(date +%Y%m%d-%H%M%S)
  local outfile=xeta_diagnostics_dump_@$timestamp.txt
  ...
  # captured cleanly
  _xeta_diag_dump_one_big_text &> "$outfile"
  if [[ $? != 0 ]]; then
  ...
  builtin echo
  builtin echo To share this with XETA developers, post it as a gist on GitHub 
  builtin echo at "https://gist.github.com" and share the link to the gist.
  builtin echo
  builtin echo "WARNING: This dump file contains all your zsh and xeta configuration files,"
  builtin echo "so don't share it publicly if there's sensitive information in them."
  builtin echo

}

function _xeta_diag_dump_one_big_text() {
  local program programs progfile md5

  builtin echo xeta diagnostic dump
  builtin echo
  ...
  builtin echo XETA_VERSION=$XETA_VERSION
  builtin echo User: $USERNAME
  builtin echo umask: $(umask)
  builtin echo
  _xeta_diag_dump_os_specific_version
  builtin echo
  ...
  _xeta_diag_dump_check_core_commands || return 1
  builtin echo  
  ...
  builtin set | command grep -a '^\(XETA\|plugins\|TERM\|LC_\|LANG\|precmd\|chpwd\|preexec\|FPATH\|TTY\|DISPLAY\|PATH\)\|XETA'
  builtin echo
  ...
  # Xeta installation
  builtin echo xeta installation:
  command ls -ld ~/.z*
  command ls -ld ~/.xe*
  builtin echo
  builtin echo xeta git state:
  (builtin cd $XETA && builtin echo "HEAD: $(git rev-parse HEAD)" && git remote -v && git status | command grep "[^[:space:]]")
  if [[ $verbose -ge 1 ]]; then
    (builtin cd $XETA && git reflog --date=default | command grep pull)
  fi
  builtin echo
  if [[ -e $XCUSTOM ]]; then
    local custom_dir=$XCUSTOM
    if [[ -h $custom_dir ]]; then
      custom_dir=$(builtin cd $custom_dir && pwd -P)
    fi
    builtin echo "xeta custom dir:"
    builtin echo "   $XCUSTOM ($custom_dir)"
  ...
  if [[ $verbose -ge 1 ]]; then
    for cfgfile in $cfgfiles; do
      _xeta_diag_dump_echo_file_w_header $cfgfile
    done
  fi
  builtin echo
  ...
  if [[ $verbose -ge 2 ]]; then
    for dumpfile in $dumpfiles; do
      _xeta_diag_dump_echo_file_w_header $dumpfile
    done
  fi

}

function _xeta_diag_dump_check_core_commands() {
  ...
  # "history" is also excluded because XETA is known to redefine that
...
function _xeta_diag_dump_echo_file_w_header() {
  local file=$1
...
function _xeta_diag_dump_os_specific_version() {
