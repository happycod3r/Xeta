...
function git_prompt_info() {
  ...
  if ! __git_prompt_git rev-parse --git-dir &> /dev/null \
     || [[ "$(__git_prompt_git config --get xeta.hide-info 2>/dev/null)" == 1 ]]; then
    return 0
  fi
...
function parse_git_dirty() {
  ...
  local -a FLAGS
  FLAGS=('--porcelain')
  if [[ "$(__git_prompt_git config --get xeta.hide-dirty)" != "1" ]]; then
...
function git_prompt_status() {
  [[ "$(__git_prompt_git config --get xeta.hide-status 2>/dev/null)" = 1 ]] && return
