...
function xeta_termsupport_precmd {
  [[ "${DISABLE_AUTO_TITLE:-}" != true ]] || return
  title "$ZSH_THEME_TERM_TAB_TITLE_IDLE" "$ZSH_THEME_TERM_TITLE_IDLE"
}

# Runs before executing the command
function xeta_termsupport_preexec {
  [[ "${DISABLE_AUTO_TITLE:-}" != true ]] || return
...
if [[ -z "$INSIDE_EMACS" || "$INSIDE_EMACS" = vterm ]]; then
  add-zsh-hook precmd xeta_termsupport_precmd
  add-zsh-hook preexec xeta_termsupport_preexec
fi
...
function xeta_termsupport_cwd {
  # Percent-encode the host and path names.
  local URL_HOST URL_PATH
  URL_HOST="$(xeta_urlencode -P $HOST)" || return 1
  URL_PATH="$(xeta_urlencode -P $PWD)" || return 1
...
add-zsh-hook precmd xeta_termsupport_cwd
