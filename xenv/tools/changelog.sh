...
function display-release {
  ...
  function fmt:hash {
    ...
    text)
      ...
      if supports_hyperlinks; then
        printf "\e]8;;%s\a%s\e]8;;\a" "https://github.com/happycod3r/xeta/commit/$hash" $text;
      else
        echo $text;
      fi ;;
    md) printf '[`%s`](https://github.com/happycod3r/xeta/commit/%s)' "$hash" "$hash" ;;
    esac
  }
  ...
  function fmt:subject {
    ...
    text)
      if supports_hyperlinks; then
        sed -E $'s|#([0-9]+)|\e]8;;https://github.com/happycod3r/xeta/issues/\\1\a\e[32m#\\1\e[0m\e]8;;\a|g' <<< "$subject"
      else
        sed -E $'s|#([0-9]+)|\e[32m#\\1\e[0m|g;s|`([^`]+)`|`\e[2m\\1\e[0m`|g' <<< "$subject"
      fi ;;
    # In markdown mode, link to (#<issue>) issues
    md) sed -E 's|#([0-9]+)|[#\1](https://github.com/happycod3r/xeta/issues/\1)|g' <<< "$subject" ;;
    esac
  }
...
function main {
  ...
  if [[ -z "$since" ]]; then
    ...
    since=$(command git config --get xeta.lastVersion 2>/dev/null) || \
